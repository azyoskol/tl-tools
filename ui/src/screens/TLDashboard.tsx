...
export const TLDashboard: React.FC = () => {
  ...
  const burndown = data?.payload?.burndown || [];
  const burndownData = burndown.map((v: number, i: number) => ({ label: `Day ${i+1}`, value: v }));
  ...
  return (
    <div className="fade-up-2" style={{ padding: 24 }}>
      ...
      {burndownData.length > 0 && (
        <Widget><h3 style={{ marginBottom: 16 }}>Sprint Burndown</h3><BarChart data={burndownData} width={600} height={200} /></Widget>
      )}
    </div>
  );
};