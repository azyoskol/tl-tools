...
export const VPDashboard: React.FC = () => {
  ...
  const prCycleTime = data?.payload?.prCycleTime || [];
  const heatmap = data?.payload?.heatmap || [];
  ...
  return (
    <div className="fade-up-2" style={{ padding: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        {prCycleTime.length > 0 && <Widget><h3 style={{ marginBottom: 16 }}>PR Cycle Time</h3><AreaChart data={prCycleTime} width={350} height={150} /></Widget>}
        <Widget><h3 style={{ marginBottom: 16 }}>Activity Heatmap</h3><Heatmap data={heatmap} width={350} height={120} /></Widget>
      </div>
    </div>
  );
};